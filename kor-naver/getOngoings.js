...
const getOngoings = async () => {
  console.log("start crawling korea naver ongoing webtoons");
  ...
  for (const day of days) {
    await page.goto("https://comic.naver.com/webtoon/weekdayList.nhn?week=" + day);
    ...
    $routeList.each(function(i) {
      let webtoonObjOverlapFlag = false;
      for (let x = 0; x<webtoon.length; x++) {
        // same webtoon already exists in array
        // add only additional published day info to the object.
        if (webtoon[x].title == ($titleList)[i].attribs.title) {
          webtoon[x].upload_day.push(day);
          webtoonObjOverlapFlag = true;
          break;
        }
      }
      if (!webtoonObjOverlapFlag) {
        webtoon[objIndex] = {
        ...
        webtoon[objIndex].upload_day.push(day);
        objIndex++;
      }
    });
  }
  ...
  fs.writeFile('./korNaverOngoing.json',json, function(err) {
    if (err == null) {
